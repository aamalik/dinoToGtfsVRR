...
'{}'||':'||version||':'||line_nr as operator_id,
line_nr as privatecode,
coalesce(line_name, '') as publiccode,
CASE WHEN (lower(veh_type_text) like '%bus%') THEN 'BUS'
     WHEN (lower(veh_type_text) like '%taxi%') THEN 'BUS'
     WHEN (veh_type_text = 'SEV') THEN 'BUS'
     WHEN (lower(veh_type_text) like '%straßenbahn%') THEN 'TRAM'
     WHEN (lower(veh_type_text) like '%strassenbahn%') THEN 'TRAM'
     WHEN (lower(veh_type_text) like '%tangobahn%') THEN 'TRAM'
     WHEN (lower(veh_type_text) like '%variobahn%') THEN 'TRAM'
     WHEN (lower(veh_type_text) like '%schwebebahn%') THEN 'METRO'
     WHEN (lower(veh_type_text) like '%u-bahn%') THEN 'METRO'
     WHEN (lower(veh_type_text) like '%bahn%') THEN 'TRAIN'
     WHEN (lower(veh_type_text) like '%zug%') THEN 'TRAIN'
     WHEN (lower(veh_type_text) like '%schiff%') THEN 'BOAT'
     WHEN (veh_type_text is NULL) THEN 'BUS'
...
null as rd_y
FROM rec_stop JOIN 
              (SELECT *, ST_setsrid(st_makepoint(stopping_point_pos_x/1000000,stopping_point_pos_y/1000000),4326) AS the_geom 
...
coalesce(SUM(st_distance(p.the_geom::geography,lp.the_geom::geography)) OVER (PARTITION BY l.version,l.line_nr,l.line_dir_nr,l.str_line_var
                                                                            ORDER BY l.line_consec_nr),0) as distancefromstart
FROM lid_course as l LEFT JOIN (SELECT *, ST_setsrid(st_makepoint(stopping_point_pos_x/1000000,stopping_point_pos_y/1000000),4326) AS the_geom FROM rec_stopping_points) as p USING (version,stop_nr,stop_type_nr,stopping_point_nr)
                     LEFT JOIN (SELECT * FROM lid_course as lp LEFT JOIN (SELECT *, ST_setsrid(st_makepoint(stopping_point_pos_x/1000000,stopping_point_pos_y/1000000),4326) AS the_geom FROM rec_stopping_points) as po USING (version,stop_nr,stop_type_nr,stopping_point_nr)) as lp
...
""", [prefix])
    ...
    for row in cur.fetchall():
        if row['operator_id'] not in timedemandgroups:
            timedemandgroups[row['operator_id']] = {'operator_id': row['operator_id'],
                                                    'privatecode': row['privatecode'], 'POINTS': [
                    {'pointorder': row['pointorder'], 'totaldrivetime': row['drivingtime'],
            ...
            totaldrivetime += row['drivingtime']
            point_dict = {'pointorder': row['pointorder'], 'totaldrivetime': totaldrivetime,
                          'stopwaittime': row['stopwaittime']}
            points.append(point_dict)
            totaldrivetime += row['stopwaittime']
    for key, row in timedemandgroups.items():
        m = md5()
        m.update(str(row['POINTS']).encode('utf-8'))
...
CASE WHEN (arr.place = dep.place) THEN %s||':'||arr.stop_nr
